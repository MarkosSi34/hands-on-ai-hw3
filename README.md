...
**Γιατί το GPTQ διατηρεί καλύτερη κατανομή βαρών (σύνδεση με τον Hessian update
rule του lab).** Το naive INT4 (NF4) στρογγυλοποιεί **κάθε βάρος ανεξάρτητα**
(round-to-nearest), οπότε τα σφάλματα κβάντισης συσσωρεύονται ασυσχέτιστα. Το GPTQ
αντίθετα κβαντίζει **στήλη-στήλη** και, μετά από κάθε στήλη, **διορθώνει τα
εναπομείναντα (ακόμα μη-κβαντισμένα) βάρη** ώστε να αντισταθμίσει το σφάλμα που μόλις
εισήχθη, το οποίο είναι ακριβώς ο update rule που υλοποιήσαμε στο lab (Part 3 / TODO 9,
`gptq_quantize`), με τον inverse Hessian `H = 2XXᵀ` να καθορίζει πόσο «ευαίσθητο»
είναι κάθε βάρος και άρα πώς να μοιραστεί βέλτιστα η διόρθωση. Έτσι η κβαντισμένη
κατανομή του GPTQ μένει **πιο κοντά στην αρχική BF16** (φαίνεται στο ιστόγραμμα:
το panel C ακολουθεί το σχήμα του A στενότερα απ' ό,τι το B), κι αυτή η διατήρηση
της κατανομής είναι που δίνει το **−4.7 % perplexity** έναντι του naive rounding.
